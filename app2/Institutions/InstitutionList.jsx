    ...
    { error && <div><p>{error}</p></div>}
    {
        ...
        <table className=" w-full bg-white table-container rounded-t-xl">
            <thead className=" bg-foreBlue text-white ">
                <th className=" border-r text-left p-4 rounded-tl-xl font-semibold">Name</th>
                <th className=" border-r text-left p-4 font-semibold">No of Faculties & Department</th>
                <th className=" text-left p-4 rounded-tr-xl font-semibold">Action</th>
            </thead>
            <tbody className="">
                {data && rows.map((item) => (
                    <tr key={item.id} className=" table-row">
                        <td className=" border-r border-t w-[25%] p-4 capitalize font-semibold text-sm">{"university of lagos"}</td>
                        <td className=" border-r w-[25%] p-4 border-t capitalize text-sm font-medium">{"12 Faculties" + " - " + "100 Departments"}</td>
                        <td className=" w-[25%] p-4 border-t">
                            <div className=' flex flex-row gap-2'>
                                <Link href={`/Institutions/Edit/${item.id}`}><svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
