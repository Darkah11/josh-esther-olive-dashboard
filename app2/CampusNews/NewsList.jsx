    ...
    { error && <div><p>{error}</p></div>}
    {
        ...
        <table className=" w-full bg-white table-container rounded-t-xl">
            <thead className=" bg-foreBlue text-white ">
                ...
                <th className=" border-r text-left p-4 font-semibold">Source</th>
                <th className=" border-r text-left p-4 font-semibold">Time</th>
            ...
            <tbody className="">
                {data && rows.map((item) => (
                    <tr key={item.id} className=" table-row">
                        ...
                        <td className=" border-r w-[25%] p-4 border-t capitalize text-sm font-medium">{"unilag news"}</td>
                        <td className=" border-r w-[25%] p-4 border-t">{item.time}</td>
                        <td className=" w-[25%] p-4 border-t">
                            <div className=' flex flex-row gap-2'>
                                <Link href={`/CampusNews/Edit/${item.id}`}><svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
